...
def generate_id(letters: list[str], params: GenerationParams):
    result = [
        ''.join(random.choices(letters, k=params.letter_count))
...
def proquint(params: GenerationParams):
    sections = []
    for _ in range(params.section_count):
        section = "".join([secrets.choice(CONSONANTS if position % 2 == 0 else VOWELS) for position in range(params.letter_count)])
...
class GenerationMethod(StrEnum):
    COUNTER = "counter", # 1, 2, ...
    ZBASE32 = "z-base-32", # https://en.wikipedia.org/wiki/Base32#cite_note-10:~:text=z%2Dbase%2D32edit
    CROCKFORD = "crockford", # https://en.wikipedia.org/wiki/Base32#cite_note-10:~:text=Crockford%27s%20Base32edit
